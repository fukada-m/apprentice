...
module RunMenu
  ...
  def run_menu2
    n = 1
    result = @conn.exec(
      "SELECT t.title, e.season_num, e.episode_num, e.episode, e.views
  ...
  def run_menu3
    @time = ''
    result = @conn.exec(
      "SELECT b.start_time, b.end_time, c.channel, t.title, e.season_num, e.episode_num, e.episode, e.episode_detail
  ...
  def run_menu4
    @time = ''
    result = @conn.exec(
      "SELECT b.start_time, b.end_time, t.title, e.season_num, e.episode_num, e.episode, e.episode_detail
                                   FROM tv_program_list AS tv
                                  INNER JOIN broadcast_times AS b ON tv.broadcast_time_id = b.broadcast_time_id
                                  INNER JOIN titles AS t ON tv.title_id = t.title_id
                                  INNER JOIN title_episode AS te ON t.title_id = te.title_id
